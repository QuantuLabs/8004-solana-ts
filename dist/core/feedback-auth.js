 ...
 */
import { PublicKey, Ed25519Program, SYSVAR_INSTRUCTIONS_PUBKEY, } from '@solana/web3.js';
/**
 ...
 * ```
 */
export function createFeedbackAuth(options) {
    const { agentId, clientAddress, indexLimit, expiryDuration = 3600, // Default: 1 hour
    chainId = 'solana-devnet', identityRegistry, signerAddress, signature, } = options;
    // Convert PublicKey to string if needed
    const clientAddressStr = typeof clientAddress === 'string'
        ? clientAddress
        : clientAddress.toBase58();
    const identityRegistryStr = typeof identityRegistry === 'string'
        ? identityRegistry
        : identityRegistry.toBase58();
    const signerAddressStr = typeof signerAddress === 'string'
        ? signerAddress
 ...
 * ```
 */
export function constructFeedbackAuthMessage(auth) {
 ...
 */
export function isFeedbackAuthValid(auth) {
    const now = Math.floor(Date.now() / 1000);
 ...
 */
export function getFeedbackAuthTimeRemaining(auth) {
    const now = Math.floor(Date.now() / 1000);
 ...
 */
export function canSubmitFeedback(auth, currentIndex) {
 ...
 * ```
 */
export function signFeedbackAuth(auth, signerSecretKey) {
    ...
    try {
        ...
        throw new Error('tweetnacl is required for signing feedbackAuth. Install it with: npm install tweetnacl');
    }
 ...
 * ```
 */
export function createFeedbackAuthEd25519Ix(feedbackAuth) {
    ...
    const signerPubkey = typeof feedbackAuth.signerAddress === 'string'
 ...
 * ```
 */
export function getInstructionsSysvarPubkey() {
    return SYSVAR_INSTRUCTIONS_PUBKEY;
}
//# sourceMappingURL=feedback-auth.js.map