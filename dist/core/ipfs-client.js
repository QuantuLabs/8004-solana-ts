            ...
            try {
                ...
                if (!verifyResponse.ok) {
                    ...
                    if (verifyResponse.status === 429) {
                        console.warn(`[IPFS] Pinata returned CID ${cid} but gateway is rate-limited (HTTP 429). ` +
                            `Content is likely available but verification skipped due to rate limiting.`);
                    }
                    else {
                        // Other HTTP errors might indicate a real problem
                        throw new Error(`Pinata returned CID ${cid} but content is not accessible on gateway (HTTP ${verifyResponse.status}). ` +
                ...
                if (verifyError instanceof Error) {
                    ...
                    if (verifyError.message.includes('timeout') || verifyError.message.includes('aborted')) {
                        console.warn(`[IPFS] Pinata returned CID ${cid} but verification timed out. ` +
                        ...
                        console.warn(`[IPFS] Pinata returned CID ${cid} but gateway is rate-limited. ` +
                        ...
                        console.warn(`[IPFS] Pinata returned CID ${cid} but verification failed: ${verifyError.message}. ` +
                            `Content may propagate with delay. Full Pinata response: ${JSON.stringify(result)}`);
                    }
                }
            }
