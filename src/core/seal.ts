 ...
 */
export function validateSealInputs(params: SealParams): void {
 ...
 */
export function computeSealHash(params: SealParams): Buffer {
...
export function computeFeedbackLeafV1(
  ...
  if (sealHash.length !== 32) throw new Error(`sealHash must be 32 bytes (got ${sealHash.length})`);
 ...
 */
export function verifySealHash(
  feedback: SealParams & { sealHash: Buffer }
): boolean {
 ...
 */
export function createSealParams(
  value: bigint,
  valueDecimals: number,
  score: number | null,
  tag1: string,
