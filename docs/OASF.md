...
The [Open Agentic Schema Framework (OASF)](https://github.com/XpressAI/oasf) provides standardized taxonomies for describing AI agent capabilities.

## SDK Functions

```typescript
import { getAllSkills, getAllDomains, validateSkill, validateDomain } from '8004-solana';

// List all valid skills/domains
const skills = getAllSkills();   // string[]
const domains = getAllDomains(); // string[]

// Validate slugs
validateSkill('natural_language_processing/summarization');  // true
validateDomain('technology/software_engineering');           // true
validateSkill('invalid_skill');                              // false
```
